...
uint8_t lps331apAddress = LPS331AP_ADDR0; //SA0=L(GND)
...
#define LPS331AP_CTRL_REG1_SETUP 0b11100100 // power:on, prs:12.5Hz, tmp:12.5Hz, BDU:no update during reading
...
float LPS331AP::ReadTemp()
{
	...
	float tempVal = 42.5 + ((float)(((int32_t)tempOutL << 8) | (int32_t)tempOutH)) / 480.0;
...
float LPS331AP::ReadPress()
{
	...
	float pressVal = ((float)(((int32_t)pressOutH << 16) | ((int32_t)pressOutL << 8) | (int32_t)pressOutXL)) / 4096.0;
