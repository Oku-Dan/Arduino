...
byte AccelGyroData[14]; //センサからのデータ格納用配列
byte MagneticData[7];	 //センサからのデータ格納用配列
byte ST1Bit;			  //磁気センサのフラッグ

void I2CRead(byte Address, byte Register, byte BytesToRead, volatile byte *Data)
{
...
void I2CWrite(byte Address, byte Register, volatile byte Data)
{
	Wire.beginTransmission(Address); //指定したアドレスと通信を始める
